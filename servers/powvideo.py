...
def test_video_exists( page_url ):
    logger.info("fusionse.servers.powvideo test_video_exists(page_url='%s')" % page_url)
...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("fusionse.servers.powvideo get_video_url(page_url='%s')" % page_url)
    ...
    for video_url in video_urls:
        logger.info("fusionse.servers.powvideo %s - %s" % (video_url[0],video_url[1]))
...
def find_videos(data):
    ...
    logger.info("fusionse.servers.powvideo find_videos #"+patronvideos+"#")
