...
def test_video_exists( page_url ):
    logger.info("fusionse.servers.vidgg test_video_exists(page_url='%s')" % page_url)
    data = scrapertools.cache_page(page_url)
    if "This file no longer exists" in data: return False, "[Vidgg] El archivo no existe o ha sido borrado"
    return True,""

def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("fusionse.servers.vidgg get_video_url(page_url='%s')" % page_url)
...
def find_videos(data):
    ...
    logger.info("fusionse.servers.vidgg find_videos #"+patronvideos+"#")
