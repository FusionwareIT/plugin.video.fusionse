...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("fusionse.servers.mp4upload get_video_url(page_url='%s')" % page_url)

    data = scrapertools.cache_page(page_url)
    logger.info("fusionse.servers.mp4upload data="+data)
    ...
    for video_url in video_urls:
        logger.info("fusionse.servers.mp4upload %s - %s" % (video_url[0],video_url[1]))
...
def find_videos(data):
    ...
    logger.info("fusionse.servers.mp4upload find_videos #"+patronvideos+"#")
