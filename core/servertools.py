...
def find_video_items(item=None, data=None, channel=""):
    logger.info("fusionse.core.servertools find_video_items")
...
def guess_server_thumbnail(title):
    logger.info("fusionse.core.servertools guess_server_thumbnail title="+title)

    lowcase_title = title.lower()

    if "netu" in lowcase_title:
        logger.info("fusionse.core.servertools guess_server_thumbnail caso especial netutv")
        return "http://media.tvalacarta.info/servers/server_netutv.png"

    if "ul.to" in lowcase_title:
        logger.info("fusionse.core.servertools guess_server_thumbnail caso especial ul.to")
        return "http://media.tvalacarta.info/servers/server_uploadedto.png"

    if "waaw" in lowcase_title:
        logger.info("fusionse.core.servertools guess_server_thumbnail caso especial waaw")
        return "http://media.tvalacarta.info/servers/server_waaw.png"

    if "streamin" in lowcase_title:
        logger.info("fusionse.core.servertools guess_server_thumbnail caso especial streamin")
        return "http://media.tvalacarta.info/servers/server_streaminto.png"

    servers = get_servers_list()
    for serverid in servers:
        if serverid in lowcase_title:
            logger.info("fusionse.core.servertools guess_server_thumbnail encontrado "+serverid)
            return "http://media.tvalacarta.info/servers/server_"+serverid+".png"

    return ""

def findvideosbyserver(data, serverid):
    logger.info("fusionse.core.servertools findvideosbyserver")
    encontrados = set()
    devuelve = []
    try:
...
def findvideos(data, skip=False):
    logger.info("fusionse.core.servertools findvideos") # en #"+data+"#")
    encontrados = set()
    devuelve = []
...
def resolve_video_urls_for_playing(server,url,video_password="",muestra_dialogo=False):
    logger.info("fusionse.core.servertools resolve_video_urls_for_playing, server="+server+", url="+url)
    video_urls = []
    torrent = False
    ...
    if server=="directo" or server=="local":
        logger.info("fusionse.core.servertools server=directo, la url es la buena")
    ...
    # Averigua las URL de los vídeos
        ...
        try:
            ...
            if muestra_dialogo:
                ...
                progreso = platformtools.dialog_progress( "fusionse" , "Connessione con "+server)
            ...
            logger.info("fusionse.core.servertools opciones disponibles para " + server + ": " + str(len(opciones)) + " "+str(opciones))
            ...
            logger.info("fusionse.core.servertools servidor de "+server+" importado")

            # Si tiene una función para ver si el vídeo existe, lo comprueba ahora
            if hasattr(server_connector, 'test_video_exists'):
                logger.info("fusionse.core.servertools invocando a "+server+".test_video_exists")
                puedes,motivo = server_connector.test_video_exists( page_url=url )

                # Si la funcion dice que no existe, fin
                if not puedes:
                    logger.info("fusionse.core.servertools test_video_exists dice que el video no existe")
                    if muestra_dialogo: progreso.close()
                    return video_urls,puedes,motivo
                else:
                    logger.info("fusionse.core.servertools test_video_exists dice que el video SI existe")
            ...
            if server_parameters["free"]=="true":
                ...
                logger.info("fusionse.core.servertools invocando a "+server+".get_video_url")
...
def get_servers_list():
  logger.info("fusionse.core.servertools get_servers_list")
