...
def is_adult(channel_name):
    logger.info("fusionse.core.channeltools is_adult channel_name="+channel_name)
...
def get_channel_parameters(channel_name):
    #logger.info("fusionse.core.channeltools get_channel_parameters channel_name="+channel_name)

    channel_xml = os.path.join(config.get_runtime_path(), 'channels', channel_name+".xml")

    if os.path.exists(channel_xml):
        #logger.info("fusionse.core.channeltools get_channel_parameters "+channel_name+".xml found")
        ...
        logger.info("fusionse.core.channeltools get_channel_parameters "+channel_name+" -> "+repr(channel_parameters) )

    else:
        logger.info("fusionse.core.channeltools get_channel_parameters "+channel_name+".xml NOT found")
...
def get_channel_json(channel_name):
    #logger.info("fusionse.core.channeltools get_channel_json channel_name="+channel_name)
    channel_xml =os.path.join(config.get_runtime_path() , 'channels' , channel_name + ".xml")
    channel_json = jsontools.xmlTojson(channel_xml)
    return channel_json['channel']
    
def get_channel_controls_settings(channel_name):    
    #logger.info("fusionse.core.channeltools get_channel_controls_settings channel_name="+channel_name)
    dict_settings= {}
    list_controls=[]
...
def get_channel_module(channel_name, package = "channels"):
    ...
    logger.info("fusionse.core.channeltools Importando " + package + channel_name)
    channels_module = __import__(package + channel_name)
    channel_module = getattr(channels_module,channel_name)
    logger.info("fusionse.core.channeltools Importado " + package + channel_name)
